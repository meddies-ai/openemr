...
//                $dosage->setText($dataRecord['drug_dosage_instructions']);
            ...
            // [MEDDIES PATCH] Populate timing.repeat from HL7 timing abbreviation
            // (BID/TID/Q6H/etc). Implements the upstream @adunsulag TODO above —
            // clinical decision support needs structured frequency/period/periodUnit
            // to compute next-dose times, not just the SNOMED abbreviation string.
            // See: https://www.hl7.org/fhir/R4/valueset-timing-abbreviation.html
            if (!empty($dataRecord['interval_codes'])) {
                $repeat = $this->buildTimingRepeatFromAbbreviation($dataRecord['interval_codes']);
                if ($repeat !== null) {
                    ...
                    $dosageArray['timing'] = $timing->jsonSerialize();
                }
            }
...
//            }
    ...
    /**
     * [MEDDIES PATCH] Maps an HL7 timing abbreviation (e.g. "BID", "Q6H") to a
     * populated FHIRTimingRepeat with frequency/period/periodUnit. Returns null
     * for abbreviations outside the standard ValueSet so the caller falls back
     * to code-only timing.
     *
     * @see https://www.hl7.org/fhir/R4/valueset-timing-abbreviation.html
     */
    private function buildTimingRepeatFromAbbreviation(string $code): ?FHIRTimingRepeat
    {
        $mapping = match (strtoupper(trim($code))) {
            'QD', 'QAM', 'QPM', 'HS', 'AM', 'PM' => [1, 1, 'd'],
            'BID'  => [2, 1, 'd'],
            'TID'  => [3, 1, 'd'],
            'QID'  => [4, 1, 'd'],
            'Q4H'  => [1, 4, 'h'],
        ...
        [$frequency, $period, $periodUnit] = $mapping;
        $repeat = new FHIRTimingRepeat();
        $repeat->setFrequency($frequency);
        $repeat->setPeriod($period);
        $repeat->setPeriodUnit(new FHIRUnitsOfTime(['value' => $periodUnit]));
        return $repeat;
    }
}
